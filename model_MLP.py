...
data_transforms = transforms.Compose([
    ...
    transforms.RandomResizedCrop(size=(20, 20)),
...
class homemade_cnn(Module):
    def __init__(self, step=1, n_case=15, batch_size=2, device=torch.device("cpu")):
        ...
        self.Lin5 = Linear(w5, w6, bias=True)
    ...
    def forward(self, input, input2, in_training=False):
        ...
        y = self.relu(self.Lin3(y))
        y = self.relu(self.Lin4(y))
        y = self.relu(self.Lin5(y))
        y = torch.flatten(self.Lin6(y).reshape(-1) + input2.reshape(-1))
        return y
    ...
    def train_loop(self, Data, loss_fn, optimizer, epoch):
        ...
        for data in Data:
            ...
            x2_data = x2_data + (0.01 * torch.randn(x2_data.shape[0]).to(self.device)-0.005)
...
def filter_data(mode):
    ...
    dict_conv = {"train": "data/master_conv_with_mean_20.trc", "test": "data/test_master_conv_with_mean_20.trc"}

    dict_ave = {"train": "temp/master_ave_dist_list_20.pkl", "test": "temp/test_master_ave_dist_list_20.pkl"}

    dict_dist = {"train": "temp/master_scan_dist_list_20.pkl", "test": "temp/test_master_scan_dist_list_20.pkl"}
