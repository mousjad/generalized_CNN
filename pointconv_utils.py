...
def prepare_data(mode):
    ...
    path = pathlib.Path(f"data/pointconv_data/all_dist_{mode}.trc")
    if path.exists():
        all_dist = torch.load(f"data/pointconv_data/all_dist_{mode}.trc").float()
        all_xyz = torch.load(f"data/pointconv_data/all_xyz_{mode}.trc").float()
        all_mean = torch.load(f"data/pointconv_data/all_mean_{mode}.trc").float()
        print(f"{mode} data is loaded")
        return all_dist, all_xyz, all_mean
        ...
        path_scan_mesh = pathlib.Path("scan_data")
        for subdir_id in tqdm(path_scan_mesh.iterdir()):
            if fid_dict[mode] in str(subdir_id):
                combs = combinations(subdir_id.iterdir(), 4)
                for comb in combs:
                    l_dist = []
                    l_xyz = []
                    for fid in comb:
                        scan_mesh = trimesh.load(fid)
                        ref_mesh_fid = "cad_model/" + subdir_id.stem + ".stl"
                        ref_mesh = trimesh.load(ref_mesh_fid).sample(n_points)
                        dist = measure_distance(scan_mesh, ref_mesh, clean=False)
                        l_dist.append(torch.tensor(dist))
                        l_xyz.append(torch.tensor(ref_mesh))
                    dist = torch.vstack(l_dist)
                    xyz = torch.stack(l_xyz, dim=0)
                    mean = (torch.vstack(l_dist).mean(axis=0) * torch.ones((len(l_dist), 1)))
        ...
        all_mean = torch.stack(all_mean, dim=0)

        torch.save(all_dist.float(), f"data/pointconv_data/all_dist_{mode}.trc")
        torch.save(all_xyz.float(), f"data/pointconv_data/all_xyz_{mode}.trc")
        torch.save(all_mean.float(), f"data/pointconv_data/all_mean_{mode}.trc")
        print(f"{mode} data is saved")
        return all_dist, all_xyz, all_mean
