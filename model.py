...
def train_generalized_CNN(ave_dist, ref_mesh, center_dist):
    ...
    ave_dist = torch.from_numpy(ave_dist).type(torch.float)
    center_dist = torch.from_numpy(center_dist).type(torch.float)

    seed = torch.manual_seed(0)
    train_gt, test_gt = torch.utils.data.random_split(center_dist, [0.75, 0.25], seed)
    ...
    for epoch in (pbar := tqdm(range(max_epoch), desc="test loss = " + str(test_loss) + " Train_loss = " + str(test_loss))):
        hmc.train()
        ...
        if epoch % 10 ==1:
            torch.save(bestmodel, wandb.run.name + 'model.trc')
